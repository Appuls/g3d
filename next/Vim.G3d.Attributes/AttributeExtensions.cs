            ...
            foreach (var attributeName in IndexingAttributes)
            {
                ...
                switch (attributeName)
                {
                    ...
                    case InstanceParentAttribute.AttributeName:
                        // numInstances:              [X],       [Y],             [Z],                   ...
                        // valueOffsets:              [0],       [X],             [X+Y],                 ...
                        // parentIndices:             [A, B, C], [D,     E,   F], [G,         H,     I], ...
                        // mergedInstanceParents:     [A, B, C], [X+D, X+E, X+F], [X+Y+G, X+Y+H, X+Y+I], ...
                        merged = collections.MergeIndexingAttribute(c => c.InstanceParentAttribute, c => c.NumInstances);
                        break;
                    case InstanceMeshAttribute.AttributeName:
                        // numMeshes:                 [X],       [Y],             [Z],                   ...
                        // valueOffsets:              [0],       [X],             [X+Y],                 ...
                        // instanceMeshes:            [A, B, C], [D,     E,   F], [G,         H,     I], ...
                        // mergedInstanceMeshes:      [A, B, C], [X+D, X+E, X+F], [X+Y+G, X+Y+H, X+Y+I], ...
                        merged = collections.MergeIndexingAttribute(c => c.InstanceMeshAttribute, c => c.NumMeshes);
                        break;
                    case MeshSubmeshOffsetAttribute.AttributeName:
                        // numSubmeshes:              [X],       [Y],             [Z],                   ...
                        // valueOffsets:              [0],       [X],             [X+Y],                 ...
                        // meshSubmeshOffsets:        [A, B, C], [D,     E,   F], [G,         H,     I], ...
                        // mergedMeshSubmeshOffsets:  [A, B, C], [X+D, X+E, X+F], [X+Y+G, X+Y+H, X+Y+I], ...
            ...
            return collections.MergeAttributes<TAttr, int>(
                ...
                tuples =>
                {
                    ...
                    foreach (var (parent, attr) in tuples)
                    {
                        ...
                        {
                            mergedData[mergedCount + i] = typedData[i] < 0
                                ? typedData[i] // if the index is less than 0, then do not increment it.
